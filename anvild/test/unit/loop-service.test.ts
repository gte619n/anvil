...
function harness(overrides?: { intakeModel?: IntakeModelFn }) {
  ...
  return { svc, env, events, progress, runEvents, notes, autopilotCalls: () => autopilotCalls, cleanup: () => { rmSync(stateDir, { recursive: true, force: true }); rmSync(repo, { recursive: true, force: true }); } };
}
