...
// (`dragging`/`justDragged` moved to sidebar.ts with the code that owns them — sidebar.ts is imported
// above, so they still initialize before this module's body runs.)
...
// hoisted declarations, and the late-declared `serverSchedule`/`closeModal` consts are wrapped in
// arrows so they're only read at call time (never during module init → no TDZ).
initFleet({
  onEvent,
  onStatus,
  sessions,
