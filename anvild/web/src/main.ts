...
function popOutReader(path: string): void {
  ...
  win.document.write(
    `<!doctype html><html lang="en" data-theme="${esc(theme)}"><head><meta charset="utf-8" />` +
      ...
      // app.css (pulled in above) pins html,body to overflow:hidden for the app shell — undo that here
      // or the popped-out document can't scroll past the first viewport.
      `<style>html,body{height:auto;min-height:100%;overflow:visible}body{margin:0;background:var(--bg);color:var(--text)}` +
